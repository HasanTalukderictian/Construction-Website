...
const Orderlist = () => {
    ...
    const [loadingOrders, setLoadingOrders] = useState(true);
    const [filterText, setFilterText] = useState("");
    const [verifyButtonLoading, setVerifyButtonLoading] = useState({});
    ...
    const verifyOrder = async (order) => {
        setVerifyButtonLoading(prev => ({ ...prev, [order.id]: true }));
        
        if (!order?.phone) {
            setVerifyError("Customer phone number is missing");
            setShowVerifyModal(true);
            setVerifyButtonLoading(prev => ({ ...prev, [order.id]: false }));
            return;
        }
    ...
    return (
        <Layout>
                                                    ...
                                                    <tr key={order.id} style={{ transition: 'all 0.3s' }} className="align-middle">
                                                        ...
                                                        <td className="px-2 px-md-3">
                                                            <div className="d-flex gap-1 justify-content-center flex-wrap">
                                                                ...
                                                                <button 
                                                                    className="btn btn-sm p-1 p-md-2" 
                                                                    style={{ 
                                                                        background: verifyButtonLoading[order.id] ? '#6b21a5' : '#8b5cf6', 
                                                                        color: 'white', 
                                                                        borderRadius: '8px', 
                                                                        width: '30px', 
                                                                        height: '30px',
                                                                    ...
                                                                    disabled={verifyButtonLoading[order.id]} 
                                                                    title="Verify"
                                                                >
                                                                    {verifyButtonLoading[order.id] ? 
            ...
            {/* ============================================ */}
            {/* ✅ SEPARATE LOADING MODAL (Shows before API response) */}
            {/* ============================================ */}
            {showLoadingModal && (
                <div 
                ...
                >
                    <div 
                        style={{
                            ...
                            padding: '45px 55px',
                            borderRadius: '24px',
                            boxShadow: '0 30px 80px rgba(139, 92, 246, 0.5), 0 0 120px rgba(139, 92, 246, 0.15)',
                            textAlign: 'center',
                            maxWidth: '420px',
                            width: '90%',
                    ...
                    >
                        ...
                        <h4 style={{ color: 'white', margin: '0 0 12px 0', fontWeight: '700', fontSize: '22px' }}>
                            Verifying Courier Data
                        </h4>
                        <p style={{ color: 'rgba(255,255,255,0.85)', margin: '0 0 25px 0', fontSize: '15px', lineHeight: '1.6' }}>
                            Please wait while we fetch the courier verification report...
                        </p>
                        ...
                        <div style={{ marginTop: '20px', color: 'rgba(255,255,255,0.6)', fontSize: '13px' }}>
                            <FaSpinner className="fa-spin me-2" size={12} />
