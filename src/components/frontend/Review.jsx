...
const Review = () => {
  ...
  useEffect(() => {
    const fetchSlides = async () => {
      try {
        const response = await fetch(`${BASE_URL}/api/get-reviews`);
        const data = await response.json();
          ...
          const apiSlides = data.data.map(slide => ({
            ...
            image: slide.image
              ? slide.image.startsWith('http')
                ? slide.image
                : `${BASE_URL}/storage/${slide.image}`
              : null,
          }));
          setSlides(apiSlides);
        } else {
          setSlides([]); // যদি data না থাকে
        }
      } catch (err) {
        console.error("Error fetching reviews:", err);
        setSlides([]); // Error হলে empty array
      }
    };
  ...
  // Duplicate slides for circular effect
  const displaySlides = [...slides, ...slides];
