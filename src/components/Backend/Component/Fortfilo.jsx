...
const Fortfilo = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const updatedBlogs = result.data.map((blog) => ({
          ...blog,
          image: blog.image ? `${BASE_URL}/storage/${blog.image}` : null,
        }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editMode) {
        response = await fetch(`${BASE_URL}/api/edit-team/${blogToEdit}`, {
          method: "POST",
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/del-team/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
          ...
          <tbody>
            {currentBlogs.map((blog, index) => (
              <tr className="text-center" key={blog.id}>
                ...
                <td className="fw-bold">{blog.Name}</td>
                <td className="fw-bold" dangerouslySetInnerHTML={{ __html: blog.Designation }} />
                <td>
                  {blog.image && (
                    <img
                      src={blog.image}
              ...
              <div className="modal-content">
                <form onSubmit={handleSubmit}>
                  <div className="modal-header">
                    <h5 className="modal-title">{editMode ? "Edit Team Member" : "Add Team Member"}</h5>
                  ...
                  <div className="modal-body">
                    ...
                    <div className="mb-3">
                      ...
                      <ReactQuill value={Designation} onChange={setDesignation} />
